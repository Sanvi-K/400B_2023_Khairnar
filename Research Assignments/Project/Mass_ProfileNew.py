...
class MassProfileNew:

    def __init__(self, galaxy, snap, merger=False):
        ...
        if merger == False:
            self.data = ReadNew(self.filename,merger=False) # for individual galaxy
        elif merger == True:
           
            self.data = ReadNew(self.filename,merger=True) # for merger remnant
    ...
    def MassEnclosed(self, ptype, radii):
        ...
        for i in range(length):
            ...
            if type(radii) == int:
                index = np.where((r_new < radii*u.kpc) & (self.data['type'] == ptype))
            else:
